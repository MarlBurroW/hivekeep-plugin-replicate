...
This plugin doubles as a real-world reference for [`@hivekeep/sdk`](https://www.npmjs.com/package/@hivekeep/sdk):

- **Three native providers in one plugin.** Same SDK contract as Anthropic / OpenAI built-ins — `LLMProvider`, `ImageProvider`, `EmbeddingProvider` from `@hivekeep/sdk`.
...
- **`ctx.http.fetch` everywhere.** No raw `globalThis.fetch`. The manifest's `http:api.replicate.com` and `http:replicate.delivery` permissions are enforced + audited by Hivekeep.
- **No Hivekeep-internal imports.** The whole module graph imports from `@hivekeep/sdk` only — exactly what a third-party plugin published on npm would look like.
